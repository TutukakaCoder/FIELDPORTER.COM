...
export function usePerformanceMonitor() {
  ...
  const getQualitySettings = useCallback((quality: QualityLevel) => {
    switch (quality) {
      case 'ultra':
        return { particleCount: 1000, shadowQuality: 'high', antialiasing: true };
      case 'high':
        return { particleCount: 500, shadowQuality: 'medium', antialiasing: true };
      case 'medium':
        return { particleCount: 250, shadowQuality: 'low', antialiasing: false };
      case 'low':
        return { particleCount: 100, shadowQuality: 'off', antialiasing: false };
      default:
        return { particleCount: 500, shadowQuality: 'medium', antialiasing: true };
    }
  }, []);
      ...
      if (frameCountRef.current % 120 === 0) {
        ...
        setMetrics(prev => {
          ...
          if (fps < 20 || memoryMB > 350) {
            ...
            if (qualityAdjustmentRef.current > 10) { // Much more conservative increase
              ...
              else if (prev.quality === 'high' && memoryMB < 150) newQuality = 'ultra';
              qualityAdjustmentRef.current = 0;
            }
          }
