  ...
  useEffect(() => {
    ...
    const detectCapability = async () => {
      ...
      if (supportsWebGL && gl) {
        ...
        if (renderer.includes('RTX') || renderer.includes('GTX 1060') || 
            renderer.includes('RX 580') || renderer.includes('M1') || 
            renderer.includes('M2') || renderer.includes('A15') || 
            renderer.includes('A16') || renderer.includes('A17')) {
          gpuTier = 'high';
        } 
        // Low-end GPU detection
        else if (renderer.includes('Mali') || renderer.includes('Adreno 3') || 
                 renderer.includes('PowerVR') || renderer.includes('Intel HD')) {
          gpuTier = 'low';
        }
      }
      ...
      if (!supportsWebGL) {
        ...
        capability = 'low';
        experience = 'css-only';
      } else if (isSmallScreen || (isMobile && cpuCores < 4) || memoryEstimate < 2 || gpuTier === 'low') {
        // Low-end devices
        capability = 'low';
        experience = 'css-only';
      } else if (isMobile || isTablet || isMediumScreen || cpuCores < 6 || memoryEstimate < 4 || gpuTier === 'medium') {
        // Mid-range devices (tablets, powerful phones, medium screens)
        capability = 'medium';
        experience = 'simplified';
      } else if (isLargeScreen && cpuCores >= 6 && memoryEstimate >= 4 && gpuTier === 'high') {
        // High-end devices
        capability = 'high';
        experience = 'full';
      }
