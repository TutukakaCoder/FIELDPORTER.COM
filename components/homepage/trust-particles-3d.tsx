...
function TrustParticles3DCore({ 
  ...
  useEffect(() => {
    ...
    particles.current = Array(totalParticles).fill(null).map((_, i) => {
  ...
  useFrame((state, delta) => {
    ...
    particles.current.forEach((particle) => {
      ...
      if (particle.lifecycle > 1) {
        ...
        particle.position.x = (particle.metricIndex - 1.5) * 2.5 + (Math.random() - 0.5) * 1.2;
        particle.position.z = (Math.random() - 0.5) * 0.3;
        needsUpdate = true;
      }
      
      // Calculate subtle opacity with gentle transitions and subtle pulse
      const pulseModifier = 1 + Math.sin(state.clock.elapsedTime * 1.5 + particle.index * 0.5) * 0.1; // Gentle pulse
...
export function TrustParticles3D({ 
  ...
  return (
    ...
    >
      ...
      >
        <TrustParticles3DCore hoveredMetricIndex={hoveredMetricIndex} />
      </Canvas>
    </div>
  );
}
