  ...
  return (
    <div className="absolute inset-0 overflow-hidden">
      ...
      <div className="absolute inset-0 opacity-40">
        ...
        <div className="absolute bottom-1/4 right-1/4 w-96 h-96 bg-emerald-500/8 rounded-full blur-3xl animate-pulse" 
             style={{ animationDelay: '1s' }} />
        <div className="absolute top-3/4 left-3/4 w-64 h-64 bg-purple-500/6 rounded-full blur-3xl animate-pulse" 
...
const IndustryCard = memo(({
  ...
  return (
    ...
    >
      <div className="relative p-6 lg:p-8 rounded-2xl bg-black/20 backdrop-blur-sm border border-white/10 hover:bg-black/30 hover:border-white/20 transition-all duration-300 h-full flex flex-col min-h-[140px] lg:min-h-[160px]">
        {/* Subtle gradient overlay */}
        <div className="absolute inset-0 bg-gradient-to-br from-white/[0.02] to-transparent rounded-2xl" />
        
        <div className="relative z-10 flex flex-col h-full items-center text-center">
          <div className={`w-10 h-10 lg:w-12 lg:h-12 mb-4 rounded-2xl bg-white/10 border border-white/20 backdrop-blur-sm flex items-center justify-center group-hover:${glowColor} transition-all duration-300`}>
            <Icon className={`w-5 h-5 lg:w-6 lg:h-6 ${iconColor}`} />
          </div>

          <div className="flex-1 space-y-2">
            <h4 className="text-base lg:text-lg font-semibold text-white group-hover:text-blue-300 transition-colors duration-300 leading-tight">
              {title}
            </h4>
...
const TestimonialCard = memo(({ testimonial, index }: { testimonial: typeof testimonials[0], index: number }) => {
  return (
    ...
    >
      ...
      <div className={`absolute inset-0 rounded-3xl opacity-0 group-hover:opacity-100 transition-opacity duration-500 ${testimonial.glowColor}`} />
      ...
      `}>
        ...
        <div className="relative z-10 flex flex-col h-full">
          <Quote className={`w-8 h-8 ${testimonial.accentColor} opacity-70 mb-6`} />

          <blockquote className="text-gray-100 leading-relaxed text-lg lg:text-xl font-light mb-8 flex-1 tracking-[-0.01em]">
          ...
          <div className="space-y-4">
            <div className="space-y-3">
              ...
              <div className={`${testimonial.accentColor} text-base font-normal`}>
                {testimonial.role}
              </div>
            </div>

            <div className="flex justify-start">
              <div className={`px-4 py-2 rounded-xl ${testimonial.badgeColor} backdrop-blur-sm`}>
...
const ProjectCard = memo(({ project, index }: { project: typeof projects[0], index: number }) => {
  ...
  return (
    ...
    >
      ...
      {/* Premium hover glow */}
      <div className={`absolute inset-0 rounded-3xl opacity-0 group-hover:opacity-100 transition-opacity duration-500 ${project.hoverGlow}`} />

      <div className={`
        relative p-6 lg:p-8 xl:p-10 rounded-3xl border border-white/10 backdrop-blur-sm
        bg-black/25 hover:bg-black/35 hover:border-white/20
        transition-all duration-300 hover:shadow-2xl
        h-full min-h-[420px] md:min-h-[480px] lg:min-h-[520px]
      `}>
        {/* Premium glassmorphism layers */}
        <div className="absolute inset-0 bg-gradient-to-br from-white/[0.03] to-transparent rounded-3xl" />
        <div className={`absolute inset-0 bg-gradient-to-br ${project.gradientFrom} ${project.gradientTo} rounded-3xl opacity-0 group-hover:opacity-100 transition-opacity duration-500`} />
        
        <div className="relative z-10 h-full flex flex-col">
          {/* Header */}
          <div className="flex items-start justify-between mb-6 lg:mb-8">
            <div className="p-4 lg:p-5 rounded-2xl bg-white/10 border border-white/20 backdrop-blur-sm group-hover:bg-white/15 transition-all duration-300">
              <project.icon className={`w-7 h-7 lg:w-8 lg:h-8 ${project.iconColor}`} />
            </div>

            <div className={`px-4 lg:px-5 py-2 lg:py-2.5 rounded-full border font-medium text-xs lg:text-sm ${project.statusBg} ${project.statusColor} backdrop-blur-sm`}>
              {project.status}
            </div>
          </div>
          ...
          <div className="flex-1 space-y-6 lg:space-y-8">
            <div className="space-y-3 lg:space-y-4">
              <h3 className="text-xl lg:text-2xl xl:text-3xl font-semibold text-white leading-tight tracking-[-0.01em]">
                {project.title}
              </h3>
              ...
              {project.keyBenefits.map((benefit, idx) => (
                <div key={idx} className="flex items-start space-x-3">
                  <div className="relative flex-shrink-0 mt-1.5">
                    <div className={`w-2 h-2 rounded-full ${project.iconColor.replace("text-", "bg-")}`} />
                    <div className={`absolute inset-0 ${project.glowColor} blur-sm opacity-60`} />
                  </div>
                  <span className="text-gray-200 leading-relaxed text-sm lg:text-base group-hover:text-white transition-colors duration-300">
                    {benefit}
                  </span>
                </div>
              ))}
    ...
    >
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-24 md:mb-28 lg:mb-32">
          <h2 className="text-4xl sm:text-5xl lg:text-6xl font-light text-white mb-12 lg:mb-16 leading-tight tracking-[-0.02em]">
            Projects We&apos;re{" "}
        ...
        <div className="mb-32 md:mb-36 lg:mb-40">
          <div className="text-center mb-24 md:mb-28 lg:mb-32">
            ...
            <p className="text-lg lg:text-xl text-gray-300 max-w-3xl mx-auto leading-relaxed font-light">
              Areas where we&apos;re developing practical solutions and gaining expertise.
            </p>
          </div>
