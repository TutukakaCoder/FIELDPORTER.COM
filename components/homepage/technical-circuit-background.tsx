...
function CircuitCameraControls({ onMouseUpdate }: { onMouseUpdate?: (position: THREE.Vector3) => void }) {
...
function DataFlowParticles({ 
  ...
  useEffect(() => {
    ...
    const serviceConnections = connections.filter((_, index) => {
      ...
      const region = serviceRegions[activeService as keyof typeof serviceRegions] || [];
    ...
    serviceConnections.slice(0, 8).forEach((connection, index) => {
      ...
      availableParticle.color = serviceColors[activeService as keyof typeof serviceColors];
  ...
  useFrame((state, delta) => {
    ...
    activeParticles.current.forEach((particle) => {
      ...
      if (particle.path.length >= 2) {
        ...
        const end = particle.path[Math.min(segmentIndex + 1, particle.path.length - 1)];
...
function TechnicalCircuitGrid({ 
  ...
  const { positions, count, connections } = useMemo(() => {
    ...
    for (let i = 0; i < positions.length; i += 3) {
      nodePositions.push(new THREE.Vector3(positions[i], positions[i + 1], positions[i + 2]));
    }
    ...
    nodePositions.forEach((node1, i) => {
      nodePositions.forEach((node2, j) => {
        ...
        if ((dx < actualSpacing * 1.1 && dz < 0.1) || (dx < 0.1 && dz < actualSpacing * 1.1)) {
  ...
  function createCircuitPath(start: THREE.Vector3, end: THREE.Vector3): THREE.Vector3[] {
  ...
  useFrame((state) => {
    ...
    const serviceColor = hoveredService 
      ? serviceColors[hoveredService as keyof typeof serviceColors] || serviceColors.default
      : serviceColors.default;
    
    connections.forEach((connection, index) => {
      if (!connection.path || connection.path.length < 2 || !connectionsRef.current) return;
      
      // Determine if this connection should be highlighted
      const isServiceConnection = hoveredService && getServiceRegion(hoveredService).includes(index % 24);
      const opacity = isServiceConnection ? 0.8 : 0.3;
      const color = isServiceConnection ? serviceColor : new THREE.Color('#3B82F6');
      
      // Create circuit path geometry
      const geometry = new THREE.BufferGeometry().setFromPoints(connection.path);
      const material = new THREE.LineBasicMaterial({
        color: color,
        opacity: opacity,
        transparent: true,
  ...
  return (
    <>
      ...
      <pointLight color="#10B981" intensity={0.4} position={[10, 5, 8]} distance={20} />
      
      <instancedMesh ref={meshRef} args={[undefined, undefined, count]} frustumCulled={false}>
        <boxGeometry args={[isMobile ? 0.12 : 0.15, 0.08, isMobile ? 0.12 : 0.15]} />
...
export function TechnicalCircuitBackground({
  ...
  if (!mounted) {
    return (
      <div className={`absolute inset-0 bg-gradient-to-br from-gray-950 via-black to-gray-950 opacity-80 ${className}`} />
    );
  }
    ...
    <div className={`absolute inset-0 ${className}`}>
      <Canvas
        ...
        onCreated={({ gl }) => {
          ...
          const handleContextLost = (event: Event) => {
            ...
            console.warn('WebGL context lost in TechnicalCircuitBackground, attempting recovery...');
            setContextLost(true);
      ...
      >
        ...
        <TechnicalCircuitGrid 
          ...
          spacing={3.5}
        />
        <CircuitCameraControls onMouseUpdate={handleMouseUpdate} />
      </Canvas>
    </div>
  );
} 