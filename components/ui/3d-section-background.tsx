...
function EnhancedCameraControls({ enabled = true, onMouseUpdate }: { enabled?: boolean, onMouseUpdate?: (position: THREE.Vector3) => void }) {
  ...
  useFrame((state) => {
    ...
    camera.position.y = THREE.MathUtils.lerp(camera.position.y, 8 + targetY, 0.008);
  ...
  useFrame((state) => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      if (distanceToMouse < mouseInfluenceRadius) {
        const mouseProximity = Math.max(0, 1 - (distanceToMouse / mouseInfluenceRadius));
        
        // More connections for better visual impact
        let connectionsCreated = 0;
        const maxConnections = 2; // Reduced for performance
        
        for (let j = i + 1; j < count && connectionsCreated < maxConnections; j++) {
          ...
          if (nodeDistance < connectionDistance) {
            const connectionStrength = Math.max(0, 1 - (nodeDistance / connectionDistance));
            const finalOpacity = mouseProximity * connectionStrength * intensity * 0.6; // Increased from 0.3
            
            if (finalOpacity > 0.02) { // Lowered threshold
              const points = [node1, node2];
              const geometry = new THREE.BufferGeometry().setFromPoints(points);
              
              const material = new THREE.LineBasicMaterial({
                color: new THREE.Color().setHSL(0.55 + mouseProximity * 0.2, 0.8, 0.5 + mouseProximity * 0.3), // Enhanced colors
...
export function SectionBackground3D({
  ...
  if (!mounted || prefersReducedMotion) {
    return (
      <div className={`absolute inset-0 bg-gradient-to-br from-gray-950 via-black to-gray-950 ${className}`} 
  ...
  return (
    <div className={`absolute inset-0 pointer-events-none ${className}`} 
      ...
      <Canvas
        ...
        onCreated={({ gl }) => {
          ...
          const handleContextLost = (event: Event) => {
            ...
            console.warn('WebGL context lost in SectionBackground3D, attempting recovery...');
            setContextLost(true);
      ...
      >
        ...
        />
        <EnhancedCameraControls enabled={enableParallax} onMouseUpdate={handleMouseUpdate} />
      </Canvas>
    </div>
  );
}
...
function ConfigurableGridWithConnections({
  ...
  useFrame((state) => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      if (!isMobile && distanceToMouse < 5) { // Increased from 4
        ...
        const floatY = Math.cos(time * 0.6 + i * 0.5) * 0.04 * (1 + attraction * 0.5);
  ...
  return (
    <><SectionNeuralConnections mousePosition3D={mousePosition3D} positions={positions} count={count} intensity={intensity} /><directionalLight position={[0, 15, 5]} intensity={intensity * 1.0} color={primaryColor} /><ambientLight intensity={0.12} color={secondaryColor} /><pointLight color={accentColor} intensity={intensity * 0.4} position={[10, 5, 8]} distance={20} /><instancedMesh ref={meshRef} args={[undefined, undefined, count]} frustumCulled={false}><sphereGeometry args={[isMobile ? 0.07 : 0.09, isMobile ? 6 : 8, isMobile ? 6 : 8]} /><MeshTransmissionMaterial color="#ffffff" thickness={0.15} roughness={0.08} transmission={0.88} ior={1.4} chromaticAberration={isMobile ? 0.004 : 0.015} anisotropy={0.05} distortion={0.05} distortionScale={0.15} temporalDistortion={isMobile ? 0.015 : 0.04} clearcoat={0.7} attenuationDistance={0.35} attenuationColor={primaryColor} envMapIntensity={0.5} samples={isMobile ? 3 : 4} resolution={isMobile ? 32 : 64} /></instancedMesh></>
  );
} 