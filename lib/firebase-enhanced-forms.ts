...
export class FirebaseEnhancedFormsService {
   ...
   */
  private calculateResourceDownloadScore(
   ...
   */
  private calculateConsultationScore(
    ...
    score +=
      COMPANY_SIZE_SCORES[
        formData.companySize as keyof typeof COMPANY_SIZE_SCORES
      ] || 2;

    // Budget scoring
    score +=
      BUDGET_SCORES[formData.budgetRange as keyof typeof BUDGET_SCORES] || 2;

    // Timeline scoring
    score +=
      TIMELINE_SCORES[formData.timeline as keyof typeof TIMELINE_SCORES] || 2;
    ...
    try {
      ...
      const submission: ResourceDownloadSubmission = {
        ...
        metadata: {
          ...
          company_domain: formData.company
            .toLowerCase()
            .replace(/[^a-z0-9]/g, ""),
        },
      };
      ...
      await this.withRetry(async () => {
        const submissionRef = doc(
          db,
          COLLECTIONS.RESOURCE_DOWNLOADS,
          submissionId,
        );
      ...
      if (typeof window !== "undefined" && (window as any).gtag) {
        (window as any).gtag("event", "resource_download_request", {
          resource_type: formData.resourceType,
  ...
  async submitGeneralInquiry(
    ...
    try {
      ...
      // Track analytics
      if (typeof window !== "undefined" && (window as any).gtag) {
        (window as any).gtag("event", "general_inquiry_submission", {
  ...
  async submitConsultationBooking(
    ...
    try {
      ...
      await this.withRetry(async () => {
        const submissionRef = doc(
          db,
          COLLECTIONS.CONSULTATION_BOOKINGS,
          submissionId,
        );
        await setDoc(submissionRef, submission);
      ...
      // Track analytics
      if (typeof window !== "undefined" && (window as any).gtag) {
        (window as any).gtag("event", "consultation_booking_request", {
          company_size: formData.companySize,
          industry: formData.industry,
          budget_range: formData.budgetRange,
          timeline: formData.timeline,
   ...
   * Validation methods
   */
  validateResourceDownload(formData: ResourceDownloadData): {
    isValid: boolean;
    errors: string[];
  } {
    const errors: string[] = [];
  ...
  validateGeneralInquiry(formData: GeneralInquiryData): {
    isValid: boolean;
    errors: string[];
  } {
    const errors: string[] = [];
  ...
  validateConsultationBooking(formData: ConsultationBookingData): {
    ...
    if (!formData.successVision.trim())
      errors.push("Success vision is required");
    ...
    if (!formData.consultationFormat)
      errors.push("Consultation format is required");
