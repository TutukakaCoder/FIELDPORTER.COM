...
const isN8nProperlyConfigured = () => {
  ...
  if (isProductionEnvironment && !isValidProductionUrl(N8N_WEBHOOK_URL)) {
    console.error('🚨 CRITICAL: Production deployment detected but n8n URL still uses localhost!');
    console.error('Please update NEXT_PUBLIC_N8N_WEBHOOK_URL in your environment variables.');
...
export interface N8nServiceError {
  type: 'network' | 'timeout' | 'rate_limit' | 'service_unavailable' | 'invalid_response';
  message: string;
  retryable: boolean;
  timestamp: Date;
}
...
export class N8nChatService {
  ...
  constructor() {
    ...
    if (!isN8nProperlyConfigured()) {
      if (isProductionEnvironment) {
        ...
        console.error('NEXT_PUBLIC_N8N_WEBHOOK_URL must be set to a valid production URL');
        ...
        console.warn('⚠️  N8N_WEBHOOK_URL not configured. AI responses will use fallback mode.');
        console.warn('For development: http://localhost:5678/webhook/fieldporter-chat');
        console.warn(
   ...
   */
  private prepareConversationContext(messages: Message[], maxMessages: number = 5): Message[] {
  ...
  private async makeN8nRequest(
    ...
    try {
      ...
      if (error.message.includes('Rate limit') && attempt < MAX_RETRY_ATTEMPTS) {
        // Wait longer for rate limit errors
        await new Promise(resolve => setTimeout(resolve, RETRY_DELAY_BASE * attempt * 2));
        return this.makeN8nRequest(payload, attempt + 1);
      }

      if (error.message.includes('Server error') && attempt < MAX_RETRY_ATTEMPTS) {
  ...
  async getChatResponse(
    ...
    const cacheResult = await responseCacheService.getCachedResponse(message, sessionId);
    ...
    try {
      ...
      const payload: N8nChatRequest = {
        ...
        sessionId,
        conversationHistory: this.prepareConversationContext(conversationHistory, 3),
      ...
      const confidence = this.calculateResponseConfidence(response.response, message);
      responseCacheService.storeCachedResponse(message, response.response, sessionId, confidence);
  ...
  getFallbackResponse(userMessage: string): string {
    ...
    const randomIndex = Math.floor(Math.random() * authenticFallbackResponses.length);
  ...
  async healthCheck(): Promise<boolean> {
    if (!isN8nProperlyConfigured()) {
      if (isProductionEnvironment) {
        console.error('🚨 Health check failed: n8n not properly configured for production');
      }
      return false;
    }
    ...
    try {
      ...
      // Enhanced logging for production health check failures
      if (isProductionEnvironment) {
        console.error('🚨 n8n health check failed in production:', {
  ...
  private calculateResponseConfidence(response: string, query: string): number {
    ...
    if (response.includes('I apologize') || response.includes("I don't have enough information")) {
      confidence -= 0.3;
    }
